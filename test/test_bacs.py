...
def test_bacs(primary_input_size, auxiliary_input_size, num_gates, num_outputs):
    ...
    for i in range(0,num_gates):
        ...
        if(i >= num_gates - num_outputs):
            ...
            var_idx = random.randint(0, (primary_input_size + auxiliary_input_size))
            var_val = pyzpk.Fp_model.one() if var_idx == 0 else all_vals[var_idx-1]
            ...
            assert(gate.evaluate(all_vals).is_zero()) # the linear combination evaluates to 0.
        else:
    ...
    assert example.circuit.is_satisfied(primary_input_list, auxiliary_input_list)